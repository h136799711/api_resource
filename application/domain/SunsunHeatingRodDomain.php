...
use app\src\sunsun\heatingRod\action\VHeatingRodTempHisAction;
...
class SunsunHeatingRodDomain extends BaseDomain
{
    ...
    public function auth(){
        ...
        $did = $this->_post('did','');
        $result = (new JudgeLegalCtrlPwdAction())->judge(new HeatingRodDeviceLogic(),$did,$ctrl_pwd);
        if($result === false){
    ...
    public function devicesCtrl(){
        ...
        if(count($entity) == 0){
            $this->apiReturnSuc(lang('device_cmd_send_success'));
        }
    ...
    public function queryHistoryTemp(){
        ...
        if(!empty($date_type)){
            ...
            switch (intval($date_type)){
                case 1:
                    //24小时
                    $result = (new VHeatingRodTempHisAction())->oneDay($did);
                    break;
                case 2:
                    //7天
                    $result = (new VHeatingRodTempHisAction())->lastNDay($did,7);
                    break;
                case 3:
                    //30天
                    $result = (new VHeatingRodTempHisAction())->lastNDay($did,30);
                    break;
                default:
