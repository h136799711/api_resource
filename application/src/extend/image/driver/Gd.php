...
class Gd{
    ...
    public function open($imgname){
        //检测图像文件
        if(!is_file($imgname))
            throw new Exception('不存在的图像文件', -3);
    ...
    public function save($imgname, $type = null, $quality=80,$interlace = true){
        if(empty($this->img)){
            ...
            throw new Exception('没有可以被保存的图像资源', -3);
        }
    ...
    public function width(){
        if(empty($this->img)){
            ...
            throw new Exception('没有指定图像资源', -3);
        }
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null){
        if(empty($this->img)){
            ...
            throw new Exception('没有可以被裁剪的图像资源', -3);
        }
    ...
    public function thumb($width, $height, $type = Image::IMAGE_THUMB_SCALE){
        if(empty($this->img))
        {
            ...
            throw new Exception('没有可以被缩略的图像资源', -3);
        }
        //原图宽度和高度
        $w = $this->info['width'];
        $h = $this->info['height'];
        ...
        switch ($type) {
            ...
            default:
                throw new Exception('不支持的缩略图裁剪类型', -3);
        }
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST,$alpha=80){
        ...
        if(empty($this->img))
        {
            ...
            throw new Exception('没有可以被添加水印的图像资源', -3);
        }
        if(!is_file($source))
        {

            throw new Exception('水印图像不存在', -3);
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if(is_array($locate)){
                    ...
                    throw new Exception('不支持的水印位置类型', -3);
                }
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        if(empty($this->img))
        {
            throw new Exception("没有可以被写入文字的图像资源", -3);
        }
        if(!is_file($font)){
            throw new Exception("不存在的字体文件：{$font}", -3);
        }
        //获取文字信息
            ...
            default:
                ...
                if(is_array($locate)){
                    ...
                    $y += $posy;
                        ...
                        throw new Exception("不支持的文字位置类型", -3);
                }
        }
